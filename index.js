...
(function(){
  ...
  if (!(isMobileDevice || isNarrow)) return;
  if (sessionStorage.getItem('mobile-modal-dismissed')) return;

  const modal = document.getElementById('mobile-modal');
  const btn = document.getElementById('mobile-modal-dismiss');
  if (!modal || !btn) return;

  // Tunda sampai preloader selesai
  const delay = sessionStorage.getItem('loaded') ? 600 : 3200;
  ...
  btn.addEventListener('click', () => {
    ...
    sessionStorage.setItem('mobile-modal-dismissed', '1');
  });
})();
...
// ── CURSOR (desktop only) ── MOBILE FIX
...
if (isTouch) {
  ...
  document.querySelectorAll('*').forEach(el => {
    if (getComputedStyle(el).cursor === 'none') el.style.cursor = 'auto';
  });
} else {
...
window.addEventListener('scroll',()=>{
  ...
  if (counter) counter.textContent=counters[cur]+' / 05';
  document.querySelectorAll('.nav-links a, #nav-mobile-menu a').forEach((a,i)=>a.classList.toggle('active',i===cur));
});
...
// ── MAGNETIC SOCIAL (desktop only) ── MOBILE FIX
if (!isTouch) {
...
// ── SPOTLIGHT (desktop only) ── MOBILE FIX
if (!isTouch) {
...
(function(){
  ...
  `;
  const striker=wrap.querySelector('#goal-striker'),ball=wrap.querySelector('#goal-ball'),keeper=wrap.querySelector('#goal-keeper'),goalText=wrap.querySelector('#goal-text');
  function runSequence() {
    striker.style.left='18%';striker.style.transform='scaleX(1)';
    ball.style.left='30%';ball.style.bottom='20px';ball.style.opacity='1';
    keeper.style.bottom='13px';keeper.style.transform='scaleX(1)';
    goalText.style.opacity='0';goalText.classList.remove('glitch');
    setTimeout(()=>{striker.style.left='26%';},500);
    setTimeout(()=>{striker.style.transform='scaleX(1) rotate(-20deg)';},600);
    setTimeout(()=>{striker.style.transform='scaleX(1) rotate(0deg)';ball.style.left='90%';ball.style.bottom='15px';},750);
    setTimeout(()=>{keeper.style.bottom='36px';keeper.style.transform='scaleX(-1) rotate(20deg)';},900);
    setTimeout(()=>{ball.style.opacity='0.3';goalText.style.opacity='1';goalText.classList.add('glitch');},1200);
    setTimeout(()=>{keeper.style.bottom='13px';keeper.style.transform='scaleX(-1) rotate(0deg)';},1700);
    setTimeout(()=>{goalText.style.opacity='0';goalText.classList.remove('glitch');},2800);
  }
  runSequence();
...
(function(){
  ...
  const inputEl=wrap.querySelector('#rpa-input'),outputEl=wrap.querySelector('#rpa-output'),coreEl=wrap.querySelector('#rpa-core'),statusEl=wrap.querySelector('#rpa-status'),progressEl=wrap.querySelector('#rpa-progress'),packetIn=wrap.querySelector('#rpa-packet'),packetOut=wrap.querySelector('#rpa-packet2');
  let taskIdx=0;
  function runTask(){
  const inp=inputs[taskIdx%inputs.length],out=outputs[taskIdx%outputs.length];taskIdx++;
  ...
  statusEl.textContent='READING';coreEl.textContent=eyes[1];
  packetIn.style.opacity='1';packetIn.style.transform='translateX(0px)';
  setTimeout(()=>{packetIn.style.transform='translateX(40px)';packetIn.style.opacity='0';},50);

  setTimeout(()=>{
    statusEl.textContent='PROCESSING';coreEl.textContent=eyes[2];
    let pct=0;progressEl.style.width='0%';
  ...
  setTimeout(()=>{
    statusEl.textContent='DONE';coreEl.textContent=eyes[3];
    if (!isMobile) {
      outputEl.innerHTML=`<div class="rpa-task-label rpa-task-out">${out}</div>`;
    }
    packetOut.style.opacity='1';packetOut.style.transform='translateX(0px)';
    setTimeout(()=>{packetOut.style.transform='translateX(40px)';packetOut.style.opacity='0';},50);
  },1500);

  setTimeout(()=>{
    statusEl.textContent='IDLE';coreEl.textContent=eyes[0];
    inputEl.innerHTML='';outputEl.innerHTML='';progressEl.style.width='0%';
  },2400);
}
  runTask();
...
function buildPreview(category){
  ...
  for(let i=0;i<Math.min(THUMB_SHOW,data.total);i++){
    const th=document.createElement('div');th.className='preview-thumb';
    const img=document.createElement('img');
    img.src=`./img/kinetic/${data.files[i]}.jpg`;img.alt=data.files[i];
    img.onerror=function(){img.style.display='none';const ph=document.createElement('span');ph.className='preview-thumb-ph';ph.textContent=i+1;th.appendChild(ph);};
    th.appendChild(img);
    const idx=i;
  ...
  if(data.total>THUMB_SHOW){
    const more=document.createElement('div');more.className='preview-more';
...
// ── LIGHTBOX ──
const lightbox=document.getElementById('lightbox'),lbImg=document.getElementById('lb-img'),lbPlaceholder=document.getElementById('lb-placeholder'),lbPlaceholderText=document.getElementById('lb-placeholder-text'),lbTitle=document.getElementById('lb-title'),lbCounter=document.getElementById('lb-counter'),lbThumbs=document.getElementById('lb-thumbs'),lbClose=document.getElementById('lb-close'),lbPrev=document.getElementById('lb-prev'),lbNext=document.getElementById('lb-next');
let lbCategory=null,lbIndex=0;

function openLightbox(category,index){lbCategory=category;lbIndex=index;renderLightbox();lightbox.classList.add('active');document.body.style.overflow='hidden';}
function renderLightbox(){
  const data=photoData[lbCategory],file=data.files[lbIndex],src=`./img/kinetic/${file}.jpg`;
  lbImg.style.display='block';lbPlaceholder.style.display='none';lbImg.src='';
  lbImg.onerror=function(){lbImg.style.display='none';lbPlaceholder.style.display='flex';lbPlaceholderText.textContent=`[ ${file}.jpg ]`;};
  lbImg.onload=function(){lbImg.style.display='block';lbPlaceholder.style.display='none';};
  lbImg.src=src;
  ...
  data.files.forEach((f,i)=>{
    ...
    const img=document.createElement('img');img.src=`./img/kinetic/${f}.jpg`;img.alt=f;
    img.onerror=function(){img.style.display='none';const ph=document.createElement('span');ph.className='lb-thumb-ph';ph.textContent=f;th.appendChild(ph);};
    th.appendChild(img);
...
// Swipe support for lightbox ── MOBILE FIX
let lbTouchStartX = 0;
...
lightbox.addEventListener('touchend', e => {
  ...
  if (Math.abs(diff) > 50) {
    ...
    lbIndex = diff > 0 ? (lbIndex+1)%data.total : (lbIndex-1+data.total)%data.total;
    renderLightbox();
  }
});
...
// ── PROJECT DATA ──
const projects=[
  {title:'Voice-to-Text Engine',category:'AI-Speech',catClass:'cat-secure',stack:'Whisper · Qwen via Ollama · Python',desc:'Engineered a localized inference pipeline for secure audio processing. Transcribes audio natively via OpenAI\'s Whisper and delivers structured content summarization using a local Qwen LLM via Ollama, ensuring zero cloud dependencies and absolute data privacy.',arch:'./img/architecture/arch_1.png',link:null},
  {title:'Fraud Detection System',category:'AI-ML',catClass:'cat-ml',stack:'Python · XGBoost · Flask',desc:'Developed a supervised machine learning pipeline utilizing XGBoost and Autoencoder models. Features specialized engineering tailored for behavioral anomaly detection and transaction amount distributions, fully deployed via a high-performance Flask API.',arch:'./img/architecture/arch_2.png',link:null},
  {title:'OCR Engine',category:'AI-Vision',catClass:'cat-secure',stack:'Azure · Ollama · Qwen · Python · Power Apps',desc:'Architected an automated OCR engine designed to streamline customer onboarding. Processes and extracts key customer attributes from high-volume, multi-format sources (10+ complex PDFs and images per profile) into structured database inputs.',arch:'./img/architecture/arch_3.png',link:null},
  {title:'Internal RAG Knowledge Base',category:'AI-GenAI',catClass:'cat-secure',stack:'Azure · Ollama · Python · Power Apps · JavaScript',desc:'Implemented an enterprise-grade Retrieval-Augmented Generation (RAG) system. Features secure document ingestion using Azure Blob Storage, semantic indexing via Azure AI Search, and LLM orchestration through Azure AI Foundry for advanced reasoning and contextual insights.',arch:'./img/architecture/arch_4.png',link:null},
  {title:'Blaugrana Analytics Dashboard',category:'Data-Visualization',catClass:'cat-ml',stack:'Python · Streamlit · Plotly',desc:'"In a way I\'m probably immortal." — Johan Cruyff. Built an advanced sports analytics platform that scrapes data from public data registries (FC Barcelona & Real Madrid FC), normalizes complex match events, and transforms them into actionable business-ready growth insights through an interactive Streamlit dashboard.',arch:'./img/architecture/arch_5.png',link:'https://blaugrana-dashboard.streamlit.app/'},
  {title:'Cross-Platform Enterprise Automation',category:'Automation',catClass:'cat-rpa',stack:'Power Automate · SQL Server · Python · VBA · AI Builder',desc:'Designed and deployed an end-to-end automation ecosystem across enterprise legacy systems. Orchestrates both attended and unattended RPA workflows using Microsoft Power Automate to eliminate manual bottlenecks and optimize data synchronization.',arch:'./img/architecture/arch_6.png',link:null}
];
...
document.querySelectorAll('.bento-card[data-project]').forEach(card=>{
  card.addEventListener('click',()=>{
    ...
    // MOBILE FIX: skip morph animation on mobile, langsung fullscreen
    const isMobile = window.innerWidth <= 768;
    if (isMobile) {
      gsap.set(expanded,{left:0,top:0,width:'100%',height:'100%',borderRadius:0,display:'block',opacity:1});
      ...
      ['es1','es2'].forEach((id,i)=>{const el=document.getElementById(id);if(el)gsap.fromTo(el,{opacity:0,y:16},{opacity:1,y:0,duration:0.5,delay:0.1+i*0.1,ease:'power2.out'});});
    } else {
      const rect=card.getBoundingClientRect();
      gsap.set(expanded,{left:rect.left,top:rect.top,width:rect.width,height:rect.height,borderRadius:'4px',display:'block'});
      expanded.classList.add('active');overlay.classList.add('active');closeBtn.classList.add('active');
      document.body.style.overflow='hidden';expandedOpen=true;
      gsap.to(expanded,{left:0,top:0,width:'100%',height:'100%',borderRadius:0,duration:0.7,ease:'cubic-bezier(0.16,1,0.3,1)',onComplete(){['es1','es2'].forEach((id,i)=>{const el=document.getElementById(id);if(el)gsap.to(el,{opacity:1,y:0,duration:0.5,delay:0.1+i*0.1,ease:'power2.out'});});}});
...
function closeExpanded(){
  ...
  document.body.style.overflow='';expandedOpen=false;
  overlay.classList.remove('active');closeBtn.classList.remove('active');
  gsap.killTweensOf(expanded);gsap.killTweensOf(overlay);
